...
class MPlan extends MObjectModel {
    ...
    public function __construct($id = 0) {
        parent::__construct();
        if($id) {
            $sqls=DB::query("SELECT id,seq_id,period,offset,UNIX_TIMESTAMP(last_ts) AS last_ts_epoch FROM plans WHERE id=$id");
            $o=$sqls->fetchObject();
            if ($o) {
                $this->id = $o->id;
                $this->seqId = $o->seq_id;
                ...
                $this->lastTs = timetostr($o->last_ts_epoch);
            }
        }
    }
    ...
    public function store(int $arg = 0) : bool {
        ...
        $sql.=" plans SET seq_id=:seq_id, period=:period, offset=:offset, last_ts=FROM_UNIXTIME(:last_ts)";
        if ($this->id) $sql.=" WHERE id=$this->id";
        $sqls=DB::prepare($sql);
        $res = true;
        try {
            $res = $sqls->execute(["seq_id" => $this->seqId, "period" => $this->period,
                "offset" => $this->offset, "last_ts" => strtotime($this->lastTs)]);
        } catch (PDOException) {
            $res = false;
        }
        if (!$res) {
