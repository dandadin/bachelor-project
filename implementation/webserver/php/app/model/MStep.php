...
class MStep extends MModel {
    ...
    /**
     * @var $nextStepId
     * Id of next step called after this step, loaded from database.
     */
    public $nextStepId;
    /**
     * @var $channelId
    ...
    public function __construct($id) {
        if($id) {
            ...
            if ($o) {
                $this->id = $o->id;
                $this->nextStepId = $o->next_step_id;
    ...
    public function store(int $arg = 0) {
        ...
        $sql.=  " steps SET next_step_id=:next_step_id, channel_id=:channel_id,".
                " value=:value, delay_before=:delay_before";
        if ($this->id) $sql.=" WHERE id=$this->id";
        $sqls=DB::prepare($sql);
        $res = true;
        try {
            $res = $sqls->execute(["next_step_id" => $this->nextStepId, "channel_id" => $this->channelId,
