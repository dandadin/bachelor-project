...
class VPageListDevices extends VPage {
    public function __construct() {
        ...
        $this->add(new VPlainTable(["ID", "Name", "Location", "Gateway", "Created At", "Last Changed At"],
            "SELECT devices.*, g.name AS gname, UNIX_TIMESTAMP(devices.created) AS dc, UNIX_TIMESTAMP(devices.last_changed) AS dlc FROM devices LEFT JOIN gateways g on devices.gateway_id = g.id".
